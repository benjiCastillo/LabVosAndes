...
class BiometriaPruebasController extends AppController
{
    ...
    public function delete($id = null)
    {
        ...
        if(!empty($user)) {
            $registry = $this->BiometriaPruebas->get($id);
            if ($this->BiometriaPruebas->delete($registry)) {
                $json = [
                    'error' => 0,
    ...
    public function printBiometriaPruebas($id = null) {
        $this->loadModel('Pruebas');
        $prueba = $this->Pruebas->find()
                        ->contain(['Pacientes', 'Medicos', 'BiometriaPruebas'])
                        ->where(['Pruebas.id' => $id])
                        ->first();
        // debug($biometria);
